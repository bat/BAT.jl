...
@testset "bat_sample" begin
    ...
    @testset "importance_sampling" begin
        function test_moments(dist::AnyDensityLike, algo::BAT.AbstractSamplingAlgorithm)
            samples = bat_sample(dist, algo).result # @inferred doesn't work

            @test isapprox(@inferred(mean(samples)), @inferred(mean(dist)), rtol=0.01)
            @test isapprox(@inferred(var(samples)), @inferred(var(dist)), rtol=0.01)
        end
