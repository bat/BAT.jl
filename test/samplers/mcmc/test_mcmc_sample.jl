...
@testset "mcmc_sample" begin
    ...
    samplingalg_MW = @inferred(TransformedMCMC(pretransform = DoNotTransform(), nchains = nchains, nsteps = nsteps))
    ...
    samplingalg_PW = @inferred TransformedMCMC(proposal = RandomWalk(), pretransform = DoNotTransform(), nsteps = 10^5, sample_weighting = ARPWeighting())
    ...
    @test BAT.sample_and_verify(Normal(), TransformedMCMC(pretransform = DoNotTransform(), nsteps = 10^4)).verified
end
