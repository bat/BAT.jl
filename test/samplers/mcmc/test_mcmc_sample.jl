...
@testset "mcmc_sample" begin
    ...
    samplingalg_MW = @inferred(TransformedMCMC(pre_transform = DoNotTransform(), nchains = nchains, nsteps = nsteps))
    ...
    @test BAT.sample_and_verify(Normal(), TransformedMCMC(pre_transform = DoNotTransform(), nsteps = 10^4)).verified
end
