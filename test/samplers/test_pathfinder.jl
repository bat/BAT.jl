...
using BAT: _lbfgs_trace, _lbfgs_inverse_hessians, _pf_factorization, _pf_lmul_L!,
    _pf_logabsdet_L, pathfinder_gaussian_fit, init_adaptive_transform,
    TriangularAffineTransform, PathfinderTransformInit, PriorApproxTransformInit
...
@testset "pathfinder" begin
    ...
    @testset "inverse Hessians and factorization" begin
        ...
        # The factorization satisfies L * Lᵀ == Σ:
        F = _pf_factorization(α, B, D)
        L = _pf_lmul_L!(F, Matrix{Float64}(I, d, d))
        @test L * L' ≈ Σ
        @test _pf_logabsdet_L(_pf_factorization(α, B, D)) ≈ logdet(Symmetric(Σ)) / 2
    end
