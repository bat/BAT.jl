...
@testset "named_prior" begin
    prior = @inferred NamedPrior(a = 5, b = Normal(), c = -4..5, d = MvNormal([1.2 0.5; 0.5 2.1]))
    ...
    @test (@inferred logpdf(prior, varshapes([0.2, -0.4, 0.3, -0.5]))) == logpdf(Normal(), 0.2) + logpdf(Uniform(-4, 5), -0.4) + logpdf(MvNormal([1.2 0.5; 0.5 2.1]), [0.3, -0.5])
