...
function run1D(
    ...
    sample_stats_all = []
    samples, chains = bat_sample(testfunctions[key].posterior, TransformedMCMC(proposal = algorithm, pretransform = DoNotTransform(), nchains = nchains, nsteps = nsteps))
    for i in 1:n_runs
        time_before = time()
        samples, chains = bat_sample(testfunctions[key].posterior, TransformedMCMC(proposal = algorithm, pretransform = DoNotTransform(), nchains = nchains, nsteps = nsteps))
        time_after = time()
...
function run2D(
    ...
    samples, stats = bat_sample(testfunctions[key].posterior, TransformedMCMC(proposal = algorithm, pretransform = DoNotTransform(), nchains = nchains, nsteps = nsteps))
    for i in 1:n_runs
        time_before = time()
        samples, stats = bat_sample(testfunctions[key].posterior, TransformedMCMC(proposal = algorithm, pretransform = DoNotTransform(), nchains = nchains, nsteps = nsteps))
        time_after = time()
