...
samples_mh = bat_sample(posterior, MCMCSampling(mcalg = MetropolisHastings(), trafo = NoDensityTransform(), nsteps = 10^5)).result
...
samples_is = bat_sample(posterior_is, MCMCSampling(mcalg = HamiltonianMC(), trafo = NoDensityTransform(), nsteps = 10^5)).result
samples_is2 = bat_sample(posterior_is2, MCMCSampling(mcalg = HamiltonianMC(), trafo = NoDensityTransform(), nsteps = 10^5)).result
