...
# "Square root" factorization of Diagonal(α) + B * D * Bᵀ (Zhang et al. 2022,
# appendix A): with U = sqrt(Diagonal(α)), QR = qr(U \ B) and
# VᵀV = I + R D Rᵀ, the left factor is L = U * Q * [Vᵀ 0; 0 I].
function _pf_factorization(α::AbstractVector, B::AbstractMatrix, D::AbstractMatrix)
    U = Diagonal(sqrt.(α))
    Q, R = qr(U \ B)
...
# x -> L * x in place, with L the left factor of the factorization above:
function _pf_lmul_L!(F, x::AbstractVecOrMat)
    k = size(F.V, 1)
    @views lmul!(F.V', x isa AbstractVector ? x[1:k] : x[1:k, :])
    lmul!(F.Q, x)
    lmul!(F.U, x)
    return x
end

_pf_logabsdet_L(F) = logdet(F.U) + logdet(F.V)
...
function pathfinder_gaussian_fit(
    ...
    Σ = muladd(best.B, best.D * best.B', Matrix(Diagonal(best.α)))
...
function _init_affine_transform(tinit::PathfinderTransformInit, target::AbstractMeasure, v_init::AbstractVector, context::BATContext)
    ...
    A = Matrix(cholesky(Positive, Σ).L)
    return MulAdd(A, μ)
end
