...
Unless set to a concrete `BATContext` object, each access
`default_batcontext[]` yields a fresh [`BATContext()`](@ref) that includes
a random number generator is seeded from `Random.default_rng()` (so
`Random.seed!` makes BAT results reproducible).

[`set_batcontext(context)`](@ref) or `default_batcontext[] = context)`
set specific `BATContext` process-wide.
`default_batcontext[] = ScopedSettings.default_value` restores the
default.

A context can also be bound for a dynamic scope only, inherited by tasks
started within that scope:
...
Also see [`get_batcontext`](@ref) and [`set_batcontext`](@ref).
"""
const default_batcontext = ScopedSetting{BATContext}(BATContext)
export default_batcontext
...
[`default_batcontext`](@ref) via `ScopedSettings.with` instead -
...
function set_batcontext(; kwargs...)
    c = get_batcontext()
    s = merge(
        (cunit=get_compute_unit(c), precision=get_precision(c), rng=get_rng(c), ad=get_adselector(c)),
        (; kwargs...)
    )
    adsel = _to_adsel(s.ad)
    set_batcontext(BATContext{s.precision}(s.rng, s.cunit, adsel, BATVisualizer())) # Temporary, to fix
end
